...
projects.forEach((p)=>{let c=document.createElement("article");c.className="card";c.innerHTML=`<div class="thumb"><video class="preview" src="${p.file}" muted playsinline preload="metadata"></video><span class="play"></span></div><div class="info"><h3>${p.title}</h3><p>${p.cat}</p></div>`;c.onclick=()=>open(p);grid.appendChild(c)});
